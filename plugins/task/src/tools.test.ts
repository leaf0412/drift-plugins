...
describe('buildTaskTools', () => {
  ...
  it('each tool has name, description, parametersSchema, and execute', () => {
    const tools = buildTaskTools(db)
  ...
  it('task_list returns tasks as JSON array', async () => {
    ...
    await createTool.execute({ title: 'Task A' })
    await createTool.execute({ title: 'Task B', status: 'done' })
  ...
  it('task_list filters by status', async () => {
    ...
    await createTool.execute({ title: 'Pending one' })
    const doneResult = await createTool.execute({ title: 'Done one' })
    const doneTask = JSON.parse(doneResult.output)
    await updateTool.execute({ id: doneTask.id, status: 'done' })

    const result = await listTool.execute({ status: 'pending' })
    expect(result.success).toBe(true)
  ...
  it('task_update changes status and returns updated task', async () => {
    ...
    const createResult = await createTool.execute({ title: 'Do stuff' })
    const created = JSON.parse(createResult.output)

    const result = await updateTool.execute({ id: created.id, status: 'done' })
    expect(result.success).toBe(true)
  ...
  it('task_update returns error for non-existent task', async () => {
    ...
    const result = await updateTool.execute({ id: 'non-existent', status: 'done' })
    expect(result.success).toBe(false)
    expect(result.error).toBeDefined()
  })
  ...
  it('task_update returns error when id is missing', async () => {
    ...
    const result = await updateTool.execute({ status: 'done' })
    expect(result.success).toBe(false)
    expect(result.error).toBeDefined()
  })
  ...
  it('task_delete removes task and returns success', async () => {
    ...
    const createResult = await createTool.execute({ title: 'Delete me' })
    const created = JSON.parse(createResult.output)

    const result = await deleteTool.execute({ id: created.id })
    expect(result.success).toBe(true)
  ...
  it('task_delete returns error for non-existent task', async () => {
    ...
    const result = await deleteTool.execute({ id: 'non-existent' })
    expect(result.success).toBe(false)
    expect(result.error).toBeDefined()
  })
})
